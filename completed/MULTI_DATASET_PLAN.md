...
- [x] Verify ~217 HC Beam Rifle / Disintegration powers in browser after the conditional-aggregation fix shipped
- [x] Decide on tooltip-level convention — resolved 2026-05-03; Sidekick keeps character-level (matches Mids). See Calc accuracy → Pending. Add a "Show game tooltip" mode only if users surface confusion.
...
- [x] Investigate the 80+ "Conditional"-labeled Rebirth pet conditional gates — fully resolved 2026-05-03 across two passes.
    - **Pass 1 (audit + converter relaxation, commit `33100f2e1`).** The literal `"Conditional"` label class was already at 0; the actual remaining noise was 122 `"state"`-labeled entries in `specialEffects[]`. Audit attributed 21 to a converter heuristic that filtered out global-scope conditionals as grant-pair candidates (relaxed) and 75 to a Parse6 parser limitation; the remaining 1 was a multi-conditional disambiguation edge case.
    - **Pass 2 (Parse6 parser fix).** Hand-decoded the Parse6 AttribMod tail for `Stalker_Melee.Claws.Strike` and confirmed the granted-power dotted name (`Temporary_Powers.Temporary_Powers.Assassins_Focus`) lives in the tail as an inline-pascal string. Added `_extract_params_parse6` in [_powers.py](tools/bin-crawler/bin_crawler/parser/_powers.py) — a Parse6-shaped tail scanner mirroring HC's `_extract_params` (scans inline-pascal strings, filters by power-name shape via the existing `_looks_like_power_name`). Plumbed through `_parse_effect_template_parse6` so the tail-scan result reaches `EffectTemplate.params`. Updated the converter's `extractSpecialEffects` Null branch to prefer `t.params.power_names[0]` over sibling-conditional pairing when present (the Parse6 tail-scan now surfaces the same data Parse7's `Grant_Power` carries directly). Re-exported Rebirth and regenerated all 271 powersets.
    - **Result.** `"state"`-labeled entries: 122 → **0**. The 122 resolved into real names — 57 "Assassins Focus", 39 "Insight", 23 "Contaminated", 3 "Pack Mentality", plus various other newly-resolved single-conditional grants. `npx tsc` clean.
...
- [x] **Rebirth picker re-enabled in production** (2026-05-03). [Header.tsx](src/components/layout/Header.tsx) `SERVER_OPTIONS` no longer flags Rebirth as `disabled`; the "(Coming Soon)" tag stays on Thunderspy only. Loadable end-to-end after Stage B + scalar-table + IO sets work landed.
- [x] **Server-switch loading UX** (2026-05-03). `handleServerChange` in [Header.tsx](src/components/layout/Header.tsx) injects a full-screen "Switching to <X>… — Loading the new dataset. This can take a moment on first switch." overlay before navigating, so the user gets immediate feedback instead of a blank reload. Post-reload, [index.html](index.html)'s static landing page reads `?serverId=` and personalizes "Loading planner..." → "Loading <X> data — this can take a moment..." for continuity until React mounts. (Vite optimizer cache also cleared after Stage B because all powerset paths changed.)
- [x] **Rebirth-only powerset icons** (2026-05-03). Wind Control: 16 `.texture` files extracted from Rebirth's `bin/data/texture_library/GUI/Icons/Powers/` via the existing pigg-wrangler `texture_to_png` pipeline → `public/img/powers/windcontrol_*.png`. Water Control: not shipped in Rebirth's distribution; user sourced 9 PNGs from the Rebirth wiki → renamed to canonical kebab-case. Kinetic Assault: 10 wiki versions overwrote earlier placeholders.
- [x] **HC Wind Control blocker** (2026-05-03). Bin re-export reintroduced Wind Control under `Controller / Dominator` for HC (it was an unfinished pre-shutdown set HC doesn't actually ship). Filtered out in [src/data/powersets.ts](src/data/powersets.ts) via a `HC_HIDDEN_POWERSETS` set so re-running `generate-powerset-index.cjs` can't reintroduce it. Wind Control still shows up under Rebirth where it's real.
- [x] **Guardian shipped for Rebirth** (2026-05-03). Investigation revealed Rebirth ships Guardian via a non-`Class_*` mechanism — `classes.bin` doesn't have `Class_Guardian` and the source tree (Source-develop/) has zero `Guardian_Assault` references, but Rebirth's actual server clearly has the AT working. The bin distribution we have at `c:/tmp/rebirth_bins/` carries the full Guardian powerset library (188 powercats incl. `Guardian_Assault` + `Guardian_Comp`); the i2401-bin-server-develop piggs are an older snapshot (186 powercats, no Guardian). Pipeline:
    - Added `Guardian_Assault` and `Guardian_Comp` to bin-crawler's `PLAYER_CATEGORIES` ([export_powers.py](tools/bin-crawler/bin_crawler/export_powers.py)) and to convert-powerset's `CATEGORY_MAP`.
    - Re-exported Rebirth and converted all 31 Guardian powersets (18 Assault + 13 Composition).
    - Added `guardian` to `ArchetypeId` ([archetype.ts](src/types/archetype.ts)) and registered the archetype in [datasets/rebirth/archetypes.ts](src/data/datasets/rebirth/archetypes.ts) with stats sourced from the Rebirth wiki: baseHP 1070.9, maxHP 2007.9, damageModifier {melee:0.95, ranged:0.95, aoe:0.85}, buffDebuffModifier 0.85, resistanceCap 0.75, damageCap 5.0. HP_CAP_GUARDIAN curve scaled from HP_CAP_STANDARD (Controller/Defender) by 1.2500 to hit the wiki L50 cap; intermediate per-level values approximate.
    - Inherent `Resolve` description threaded in (defeat-driven absorb + status protection, 5% per stack up to 3, 20% on PvP defeat).
    - Header dropdown updated: `ARCHETYPE_OPTIONS_REBIRTH` Guardian entry no longer disabled / "Coming Soon".
    - Powerset index regenerated (271 → 302 sets).
    - `npx tsc` clean.

  Open follow-ups (not blocking):
    - HP cap intermediate values are scaled approximations; live Rebirth per-level data would refine them.
    - Resolve isn't a binary-encoded conditional template (engine-level mechanic), so it doesn't surface as a Mechanic Adjuster toggle. If/when calc-time Absorb modeling lands, Resolve gets a Header-state slider similar to Fury/Domination.
    - Guardian's full damage modifier table (Heal, Defense Buff, ToHit Buff, etc., per the wiki's full mod table) is captured only at the `damageModifier`/`buffDebuffModifier` summary level. If specific stats display as off vs Mids/in-game, refine the per-stat scalars.
