...
- [x] **Dominator Shadow Field / Mirage rendered NOTHING by default** — follow-up to TSPY10, *different* root cause (converter, not vocabulary). The Dominator versions split their summon into complementary `kStealth source> 0.5 <=` (Domination OFF = base/default) and `> 0.5` (Domination ON = `_Domination` variant) branches. `convert-powerset.cjs` `_isConditionalGate` treated the `<=` (Domination-inactive) branch as conditional too (`kStealth source>` → the Dominator "Domination Active" toggle), so the BASE summon was swallowed into `conditionalEffects[domination]` and `effects.summon` was empty — with Domination off (default) the InfoPanel (reads `effects.summon`) showed no pet block and no hoisted control/debuff. Fix: `_isConditionalGate` treats a bare `kStealth source> N <=` as the base case (like an RPN `!`); `$`-anchored so a `<=` chained with a further positive gate stays conditional. Base summon → `effects.summon` (shows by default), `>` variant stays the conditional. Blast radius = 3 powers (tspy Shadow Field + Mirage, **Rebirth** Shadow Field had the same latent bug → also fixed).   verify: fn:_isConditionalGate, file:src/data/thunderspy-pet-summon.test.ts
