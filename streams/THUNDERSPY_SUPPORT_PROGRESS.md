...
- [ ] **TSPY10** — pseudo-pet **redirect** resolution for tspy shells: Caltrops / Burn / Disintegrate-spread / rains carry their damage in `Redirects.*` powers, not an entity record, so their summon links but shows no damage (part of the ~45% of summon entity_defs still name-only, alongside out-of-scope `IncarnatePets_*` which tspy — level cap 50, no Incarnate — never reaches). HC's `attachResolvedPseudoPets` resolves these; it keys on shells in `PSEUDOPET_SHELL_ENTITIES` and the `params.redirects` list (now emitted by the summon fix), so wiring tspy is mostly a shell-allowlist + redirect-path check.
