...
- **Batch 3 (a65727917):** the 26 held EAT-epic powers, sourced directly from the binary. Ran the Bin Crawler on the live `.pigg` for the 5 relevant categories → **405/405 identical** to committed exported_powers (generated is current with the live game). Generated is the richer/correct effect set; overrides were stale CoD2 subsets. Verified the §3 protection-vs-resistance split is handled correctly (tactical-training confuse: Current -15 → protection, Resistance 1.75/3.25 → mezResistance). Stripped stale conflicts; **preserved genuine enrichments** the parser doesn't capture (discharge `summon.copyBoosts` — a consumed field / §1 parser gap; PvE/PvP stealth supersets).
- **DIVERGENT 140 → 9.** The 9 remaining are all correctly-preserved override *enrichments* (parser-gap corrections + supersets, audit-label false-positives like assassins-strike), NOT stale data. **MSOT-4 Step 2 — DONE.**
- **Parser-gap follow-up (optional, data-driven per §0):** the bin parser drops `copy_boosts` from summon templates, so pets' enhancement-application currently relies on hand overrides (discharge). Extracting it at the parser would let those overrides go away too. Logged in BIN-PARSER-LOG.md.
