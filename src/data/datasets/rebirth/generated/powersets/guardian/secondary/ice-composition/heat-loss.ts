/**
 * Heat Loss — GENERATED LAYER
 * AUTO-GENERATED by `node scripts/convert-powerset.cjs guardian_comp ice_composition`.
 * Do not hand-edit. Manual deltas go in the parallel overrides file.
 *
 * Source: guardian_comp/ice_composition/heat_loss.json
 */

import type { Power } from '@/types';

export const HeatLoss: Power = {
  "name": "Heat Loss",
  "internalName": "Heat_Loss",
  "available": 37,
  "autoIssue": false,
  "free": false,
  "description": "Heat Loss drains the heat from your enemies and transfers that energy to your allies in the form of Endurance. All foes near the selected target may experience a Heat Loss, which will drain their Endurance, Slow them, and reduce their Damage Resistance. Allies near the target are granted Endurance and a boost to their Recovery. Some Endurance and Recovery will also be transferred directly to you and any allies near you. Recharge: Long",
  "shortHelp": "Ranged (Foe AoE), Foe -RES, -END, Slow; Team +END, +Recovery",
  "icon": "icecomp_heatloss.png",
  "powerType": "Click",
  "targetType": "Foe",
  "effectArea": "SingleTarget",
  "targetsAffected": [
    "Foe"
  ],
  "stats": {
    "accuracy": 1,
    "range": 70,
    "recharge": 360,
    "endurance": 15.6,
    "castTime": 2.17
  },
  "allowedEnhancements": [
    "EnduranceModification",
    "Slow",
    "EnduranceReduction",
    "Range",
    "Recharge",
    "Accuracy"
  ],
  "allowedSetCategories": [
    "Endurance Modification",
    "Slow Movement",
    "Universal Debuff"
  ],
  "maxSlots": 6,
  "effects": {
    "summon": {
      "duration": 2,
      "entities": [
        {
          "count": 1,
          "entity": "Pets_HeatLossDebuff"
        },
        {
          "count": 1,
          "entity": "Pets_HeatLossBuff"
        }
      ],
      "isPseudoPet": false
    }
  },
  "atoms": [
    ["EntCreate",null,-1,1,2,"Ranged_Ones","Cur","Magnitude","Target","Any",false,"Stack",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,null,null,null,null,"Ones"],
    ["EntCreate",null,-1,1,2,"Ranged_Ones","Cur","Magnitude","Self","Any",false,"Stack",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,null,null,null,null,"Ones"]
  ]
};
