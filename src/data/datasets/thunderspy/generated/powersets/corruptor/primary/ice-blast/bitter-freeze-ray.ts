/**
 * Bitter Freeze Ray — GENERATED LAYER
 * AUTO-GENERATED by `node scripts/convert-powerset.cjs corruptor_ranged ice_blast`.
 * Do not hand-edit. Manual deltas go in the parallel overrides file.
 *
 * Source: corruptor_ranged/ice_blast/bitter_freeze_ray.json
 */

import type { Power } from '@/types';

export const BitterFreezeRay: Power = {
  "name": "Bitter Freeze Ray",
  "internalName": "Bitter_Freeze_Ray",
  "available": 25,
  "description": "This power can Hold your opponent frozen solid in a block of ice. The victim can be attacked and will remain frozen and helpless. After the ice thaws, the victim emerges chilled and Slowed.  Bitter Freeze Ray deals medium damage. Damage: Moderate, Recharge: Slow",
  "shortHelp": "Ranged, Moderate DMG(Cold), Foe Hold",
  "icon": "iceblast_bitterfreezeray.png",
  "powerType": "Click",
  "targetType": "Foe",
  "effectArea": "AoE",
  "stats": {
    "accuracy": 1,
    "range": 80,
    "radius": 20,
    "recharge": 20,
    "endurance": 15.184,
    "castTime": 2.5
  },
  "allowedEnhancements": [
    "Hold",
    "Slow",
    "EnduranceReduction",
    "Range",
    "Recharge",
    "Damage",
    "Accuracy"
  ],
  "allowedSetCategories": [
    "Corruptor Archetype Sets",
    "Holds",
    "Ranged Damage",
    "Sleep",
    "Slow Movement",
    "Universal Damage Sets"
  ],
  "maxSlots": 6,
  "damage": {
    "type": "Cold",
    "scale": 1.32,
    "table": "Ranged_Damage"
  },
  "effects": {
    "hold": {
      "mag": 3,
      "scale": 8,
      "table": "Ranged_Immobilize"
    },
    "sleep": {
      "mag": 3,
      "scale": 30,
      "table": "Ranged_Sleep"
    }
  },
  "atoms": [
    ["Unmapped",null,1.32,1,0,"Ranged_Damage","Abs","Magnitude","Target","Any",true,"No",null,null,null,1],
    ["Unmapped",null,0.2,1,18,"Ranged_Slow","Str","Magnitude","Target","Any",true,"No",null,null,null,1],
    ["Mez","Held",8,3,0,"Ranged_Immobilize","Cur","Duration","Target","PvE",true,"No",null,null,null,1,null,null,null,null,null,null,"enttype target> critter eq"],
    ["Mez","Sleep",30,3,0,"Ranged_Sleep","Cur","Duration","Target","PvE",true,"No",null,null,null,1,null,null,null,null,null,null,"enttype target> critter eq"],
    ["Unmapped",null,100,1,10,"Ranged_Ones","Res","Magnitude","Target","Any",true,"No",null,null,null,1],
    ["Unmapped",null,1.32,1,0,"Ranged_Damage","Abs","Magnitude","Target","PvE",true,"No",null,null,null,1,null,null,null,null,null,null,"kHitPoints% target> 10 - 100 * 50 10 - / 0 100 minmax rand 100 * < enttype target> critter eq &&",true],
    ["Unmapped",null,1.32,1,0,"Ranged_Damage","Abs","Magnitude","Target","PvP",true,"No",null,null,null,1,null,null,null,null,null,null,"kHitPoints% target> 10 - 100 * 50 10 - / 0 100 minmax rand 100 * < enttype target> player eq &&",true],
    ["Mez","Held",2,3,0,"Ranged_Ones","Cur","Duration","Target","PvP",true,"No",null,null,null,1,null,null,null,null,null,null,"enttype target> player eq",true],
    ["Mez","Sleep",2,3,0,"Ranged_Ones","Cur","Duration","Target","PvP",true,"No",null,null,null,1,null,null,null,null,null,null,"enttype target> player eq",true]
  ],
  "damageTypes": [
    "Cold"
  ]
};
