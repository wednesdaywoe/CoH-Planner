/**
 * Blinding Powder — GENERATED LAYER
 * AUTO-GENERATED by `node scripts/convert-powerset.cjs blaster_support ninja_training`.
 * Do not hand-edit. Manual deltas go in the parallel overrides file.
 *
 * Source: blaster_support/ninja_training/blinding_powder.json
 */

import type { Power } from '@/types';

export const BlindingPowder: Power = {
  "name": "Blinding Powder",
  "internalName": "Blinding_Powder",
  "available": 27,
  "description": "You throw a handful of Blinding powder in a wide arc at your foes. Most foes will be blinded, and unable to see. Some affected targets may be overcome by the powder that they may start attacking their own allies. If you attack the blinded foes, they will be alerted to your presence, but will continue to suffer a penalty to their chance to hit.Notes: The Sleep component of this power is Auto Hit against regular enemies, but a To Hit check is required to against AVs and players, as well as to make secondary effects apply.Recharge: Long.",
  "shortHelp": "Ranged (Cone), Foe -To Hit, Sleep, Confuse, -Perception",
  "icon": "ninjatools_blindingpowder.png",
  "powerType": "Click",
  "targetType": "Foe",
  "effectArea": "Cone",
  "stats": {
    "accuracy": 0.8,
    "range": 25,
    "radius": 25,
    "arc": 0.7853981852531433,
    "recharge": 90,
    "endurance": 7.8,
    "castTime": 1.07,
    "maxTargets": 10
  },
  "allowedEnhancements": [
    "EnduranceReduction",
    "Range",
    "Recharge",
    "ToHit Debuff",
    "Confuse",
    "Accuracy"
  ],
  "allowedSetCategories": [
    "Accurate To-Hit Debuff",
    "Confuse",
    "To Hit Debuff"
  ],
  "maxSlots": 6,
  "effects": {
    "buffDuration": 20,
    "confuse": {
      "mag": 3,
      "scale": 8,
      "table": "Melee_Immobilize"
    },
    "durations": {
      "perceptionDebuff": 20,
      "tohitDebuff": 15
    },
    "perceptionDebuff": {
      "scale": 0.9,
      "table": "Melee_Ones"
    },
    "sleep": {
      "mag": 3,
      "scale": 10,
      "table": "Melee_Sleep"
    },
    "tohitDebuff": {
      "scale": 1,
      "table": "Melee_DeBuff_ToHit"
    }
  },
  "atoms": [
    ["ToHit",null,1,1,15,"Melee_DeBuff_ToHit","Cur","Magnitude","Target","Any",true,"Replace",2,null,null,1],
    ["Mez","Sleep",10,3,0,"Melee_Sleep","Cur","Duration","Target","Any",true,"Replace",2,null,null,1],
    ["Mez","Confused",8,3,0,"Melee_Immobilize","Cur","Duration","Target","Any",true,"Replace",2,null,null,0.5],
    ["Perception",null,-0.9,1,20,"Melee_Ones","Cur","Magnitude","Target","Any",true,"Replace",2,null,null,1],
    ["DamageBuff","Smashing",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Lethal",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Fire",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Cold",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Energy",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Negative",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Psionic",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["DamageBuff","Toxic",0,0,0,"Ranged_Ones","Str","Expression","Self","Any",false,"Replace",2,null,null,1,null,true,null,null,null,null,null,null,null,null,null,null,".066 activatetime power.base> * areafactor power.base> /"],
    ["Mez","Sleep",10,3,0,"Melee_Sleep","Cur","Duration","Target","Any",true,"Replace",2,null,null,1,null,null,null,null,null,null,"Raid target.HasTag?",true],
    ["Perception",null,-0.9,1,20,"Melee_Ones","Cur","Magnitude","Target","Any",true,"Replace",2,null,null,1,null,null,null,null,null,null,"Raid target.HasTag?",true],
    ["Mez","Confused",8,3,0,"Melee_Immobilize","Cur","Duration","Target","Any",true,"Replace",2,null,null,0.5,null,null,null,null,null,null,null,true],
    ["Mez","Sleep",1,3,0,"Melee_PvPMez","Cur","Duration","Target","Any",true,"Stack",2,null,null,1,null,null,null,null,null,null,"enttype target> player eq",true],
    ["Perception",null,-0.9,1,20,"Melee_Ones","Cur","Magnitude","Target","Any",true,"Replace",2,null,null,1,null,null,null,null,null,null,"enttype target> player eq",true],
    ["Mez","Confused",1,3,0,"Melee_PvPMez","Cur","Duration","Target","Any",true,"Replace",2,null,null,0.5,null,null,null,null,null,null,null,true]
  ],
  "specialEffects": [
    {
      "kind": "effect-proc",
      "chance": 0.5,
      "label": "Confuse"
    }
  ]
};
