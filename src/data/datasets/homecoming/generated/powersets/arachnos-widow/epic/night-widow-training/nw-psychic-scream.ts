/**
 * Psychic Scream — GENERATED LAYER
 * AUTO-GENERATED by `node scripts/convert-powerset.cjs widow_training night_widow_training`.
 * Do not hand-edit. Manual deltas go in the parallel overrides file.
 *
 * Source: widow_training/night_widow_training/nw_psychic_scream.json
 */

import type { Power } from '@/types';

export const NWPsychicScream: Power = {
  "name": "Psychic Scream",
  "internalName": "NW_Psychic_Scream",
  "available": 23,
  "description": "This howl of Psionic energy resonates in the minds of all foes within its conical area of effect, inflicting moderate damage.",
  "shortHelp": "Ranged (Cone), DMG(Psionic), Foe -Recharge",
  "icon": "nightwidowtraining_psychicscream.png",
  "powerType": "Click",
  "targetType": "Foe",
  "effectArea": "Cone",
  "stats": {
    "accuracy": 1,
    "range": 50,
    "radius": 50,
    "arc": 0.7853981852531433,
    "recharge": 20,
    "endurance": 21.157,
    "castTime": 2,
    "maxTargets": 12
  },
  "allowedEnhancements": [
    "EnduranceReduction",
    "Range",
    "Recharge",
    "Damage",
    "Accuracy"
  ],
  "allowedSetCategories": [
    "Ranged AoE Damage",
    "Soldiers of Arachnos Archetype Sets",
    "Universal Damage Sets"
  ],
  "maxSlots": 6,
  "damage": {
    "type": "Psionic",
    "scale": 1.4931,
    "table": "Ranged_Damage"
  },
  "effects": {
    "buffDuration": 20,
    "durations": {
      "rechargeDebuff": 20
    },
    "rechargeDebuff": {
      "scale": 0.5,
      "table": "Ranged_Slow"
    }
  },
  "atoms": [
    ["Damage","Psionic",1.4931,1,0,"Ranged_Damage","Abs","Magnitude","Target","Any",true,"Stack",2,null,null,1,null,null,null,null,null,null,"enttype target> critter eq"],
    ["RechargeTime",null,0.5,1,20,"Ranged_Slow","Str","Magnitude","Target","Any",true,"Stack",2,null,null,1,null,true],
    ["Damage","Psionic",0.9541,1,0,"Ranged_PvPDamage","Abs","Magnitude","Target","Any",true,"Stack",2,null,null,1,null,null,null,null,null,null,"enttype target> player eq"],
    ["Damage","Psionic",1.6459,1,0,"Ranged_PvPDamage","Abs","Magnitude","Target","Any",true,"Stack",2,null,null,1]
  ]
};
