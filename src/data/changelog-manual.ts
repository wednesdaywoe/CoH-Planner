...
export const MANUAL_CHANGELOG_GROUPS: ManualChangelogGroup[] = [
  // ───────────────────────────────────────────────────────────────────────
  {
    ...
    items: [
      ...
      { id: 'specialized-pool-exclusion-five', message: 'The game allows only one Specialized power pool per build, and the planner was only enforcing that across three of the five: Gadgetry and Utility Belt were missing, so a Rebirth or Thunderspy build could take two or three of them at once. All five are mutually exclusive now. Homecoming was never affected, since it does not release those two pools.', type: 'fix' },
      { id: 'incompatible-powerset-pairs', message: 'Some primary/secondary combinations are impossible in game — Titan Weapons with Shield Defense, Claws with Stone Armor, and others. The planner let you pick them, then quietly offered no level 1 powers at all, leaving you stuck with no explanation. Those combinations are now greyed out in the Primary and Secondary dropdowns and say which set they clash with.', type: 'fix' },
      ...
      { id: 'veat-branch-switch-strips', message: 'An Arachnos Soldier or Widow gets one specialization, but switching the Branch dropdown left the old branch\'s powers in the build — a Bane Spider\'s Shatter still counted in your totals on a Crab Spider, with no way to take it off. Changing branch now removes the powers that branch owned, tells you which went, and offers an Undo.', type: 'fix' },
    ]
  },
  {
    date: '2026-08-05',
