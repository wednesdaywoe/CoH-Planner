...
export const MANUAL_CHANGELOG_GROUPS: ManualChangelogGroup[] = [
  // ───────────────────────────────────────────────────────────────────────
  {
    ...
    items: [
      { id: 'kheldian-form-attack-chain', message: 'The attack chain builder can finally see Kheldian form attacks. Nova and Dwarf attacks cost no power pick, and the builder was dropping every power it didn\'t have to pay for — so a Kheldian had nothing to build a chain out of. They\'re offered now, gated by form: you get the form\'s own attacks rather than a mix of Nova, Dwarf and human powers you couldn\'t use in one sequence. The Form control shows up only when your build can actually change form, so a Stone Armor build doesn\'t get a pointless one.', type: 'fix' },
      { id: 'attack-chain-empty-character', message: 'The attack chain builder was doing its math on an empty character. It rebuilt your totals from scratch with every slider off, no incarnates and no exemplar, instead of reading the same numbers as the dashboard. That\'s why the Rage stack slider\'s "Off" position still counted a stack, and why a second stack of Rage reached your totals but never the chain.', type: 'fix' },
      { id: 'tohit-buff-self-enhancing', message: '+ToHit buffs were enhancing themselves. The to-hit row read your character\'s accumulated ToHit as its own strength — which already contained the row being drawn — so two stacks of Rage showed their own +40% as 56%. Nothing downstream believed it, so the number you saw disagreed with everything calculated from it. Compare Slotting was the one surface never applying the strength pass at all; it folds it in now, so all three agree.', type: 'fix' },
      { id: 'generic-io-value-display', message: 'Generic IOs were advertising values they don\'t deliver. The picker carried its own hand-written Schedule A curve while the calc read the real one, so seven of the 25 generic types disagreed: ToHit, ToHit Debuff, Defense, Resistance and Range all showed 42.4% and applied 25.5%; Interrupt showed 42.4% and applied 50.9%; Knockback showed 42.4% and applied 76.4%. The duplicate table is gone. One number can\'t describe 25 aspects across four schedules, so the level moved to the header and each stat\'s real value now lives in its own tooltip.', type: 'fix' },
      { id: 'unmapped-enhancement-aspects', message: 'Immobilize, Terrorize, Threat and Interrupt enhancements were contributing nothing whatsoever. Those names were missing from the aspect map, and a missing name isn\'t an inert one — every consumer just skipped it. 109 Homecoming powers and 51 set pieces were affected. There\'s a test now that walks every server\'s enhancement vocabulary, so the next unmapped term fails loudly instead of silently zeroing.', type: 'fix' },
      { id: 'tspy-pets-summoned-twice', message: 'Thunderspy: summons were exporting their pets twice, so anything that calls in help — Spiderlings, Call Reinforcements, Omega Maneuver and ~350 others — was counting the same pet more than once. Pet contributions on those builds will come down to what the game actually summons.', type: 'fix' },
      { id: 'tspy-range-disallowed', message: 'Thunderspy: 720 powers now say which enhancement strengths they refuse. Mostly this is +Range on melee attacks, which the planner had been happily applying to powers the game does not let you extend.', type: 'fix' },
      { id: 'tspy-mechanic-names', message: 'Thunderspy: power mechanics now carry their real names instead of a placeholder. Contaminated Strike says "Contaminated" rather than "state", and about 170 powers read the same way.', type: 'fix' },
      { id: 'rebirth-stealth-suppression', message: 'Rebirth: stealth and suppression data was being dropped before it reached the planner. Cloaking Device, Personal Force Field, Invisibility, Shadow Fall, Arctic Fog, Energy Cloak, Mask Presence and ~70 others were missing the part of their record that says what suppresses in combat. It reads now.', type: 'fix' },
