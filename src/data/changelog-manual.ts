...
export const MANUAL_CHANGELOG_GROUPS: ManualChangelogGroup[] = [
  // ───────────────────────────────────────────────────────────────────────
  {
    ...
    items: [
      { id: 'scrapper-crit-own-rows', message: 'Scrapper critical damage was shown as a flat +10% average on every attack, and the game does not work that way: each power carries its own crit effect with its own damage and chance. One Thousand Cuts crits for 138.26 at level 50 (the planner showed +14.77), and Sweeping Strike rolls 15%, not 10%. The "w/ Crit" column now shows what a critical hit actually deals — the power\'s own crit damage added to the final — and the Critical Hits card lists the real chance and damage for both the vs-minion and vs-lieutenant-and-up branches, straight from the game data.', type: 'fix' },
