 ...
 *
 * Falls back to the `id` slug for records with no `internalName` — power POOLS, which
 ...
 */
export function setKeyFromId(id: string | undefined, internalName?: string): string | undefined {
...
export function heldPowersetNames(sets: (Powerset | undefined)[]): Set<string> {
  return new Set(
    sets
      .map((s) => setKeyFromId(s?.id, s?.internalName))
