...
export class DatasetHandle {
     ...
     * @param {string} powerset
     * @param {string} internal_name
     * @param {number | null} [targets_hit]
     * @returns {string}
     */
    project_power(build_json, powerset, internal_name, targets_hit) {
        let deferred5_0;
        let deferred5_1;
        try {
            const ptr0 = passStringToWasm0(build_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len0 = WASM_VECTOR_LEN;
            const ptr1 = passStringToWasm0(powerset, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len1 = WASM_VECTOR_LEN;
            const ptr2 = passStringToWasm0(internal_name, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len2 = WASM_VECTOR_LEN;
            const ret = wasm.datasethandle_project_power(this.__wbg_ptr, ptr0, len0, ptr1, len1, ptr2, len2, isLikeNone(targets_hit) ? Number.MAX_SAFE_INTEGER : (targets_hit) >>> 0);
            var ptr4 = ret[0];
            var len4 = ret[1];
            if (ret[3]) {
                ptr4 = 0; len4 = 0;
     ...
     * @returns {string}
     */
    recalculate(build_json) {
        let deferred3_0;
        let deferred3_1;
        try {
            const ptr0 = passStringToWasm0(build_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
            const len0 = WASM_VECTOR_LEN;
            const ret = wasm.datasethandle_recalculate(this.__wbg_ptr, ptr0, len0);
            var ptr2 = ret[0];
            var len2 = ret[1];
            if (ret[3]) {
                ptr2 = 0; len2 = 0;
 ...
 */
export function load_dataset(bytes) {
    const ptr0 = passArray8ToWasm0(bytes, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.load_dataset(ptr0, len0);
    if (ret[2]) {
 ...
 * @returns {string}
 */
export function what_if_vocabulary() {
    let deferred1_0;
    let deferred1_1;
    try {
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./coh_wasm_bg.js": import0,
    };
}

const DatasetHandleFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_datasethandle_free(ptr, 1));
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('coh_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
