 ...
 * Audit Pool Powers
 *
 * Compares processed pool power data (power-pools-raw.ts) against
 * raw Homecoming JSON data for accuracy.
 *
 * Usage:
 *   node scripts/audit-pool-powers.cjs                    # All pools
 *   node scripts/audit-pool-powers.cjs --pool fighting    # One pool
 *   node scripts/audit-pool-powers.cjs --verbose          # Show PASS results
 *   node scripts/audit-pool-powers.cjs --json             # JSON output
 */
...
// ============================================
// LOAD PROCESSED POOL DATA
// ============================================

function loadProcessedPools() {
  const filePath = path.resolve('./src/data/power-pools-raw.ts');
  const content = fs.readFileSync(filePath, 'utf-8');

  // Extract the POWER_POOLS_RAW object (may end with "as const;")
  const match = content.match(/export\s+const\s+POWER_POOLS_RAW\s*=\s*(\{[\s\S]*\})\s*(?:as\s+const)?\s*;?\s*$/);
  if (!match) {
    console.error('Could not parse power-pools-raw.ts');
    process.exit(1);
  }
  ...
  try {
    ...
    console.error('Failed to parse POWER_POOLS_RAW:', e.message);
    process.exit(1);
  }
}

// ============================================
// MAP POOL POWER TO RAW JSON
// ============================================

/**
 * Derive raw JSON path from fullName
 * "Pool.Fighting.Boxing" → "pool/fighting/boxing.json"
 */
function getRawJsonPath(fullName) {
  const parts = fullName.split('.');
  if (parts.length !== 3 || parts[0] !== 'Pool') return null;

  const poolDir = parts[1].toLowerCase();
  const powerFile = parts[2].toLowerCase() + '.json';
  return path.join(RAW_POWERS_PATH, 'pool', poolDir, powerFile);
}
...
function compareEffects(rawJson, processedEffects) {
  ...
  // Check effects (skip meta keys and low-priority)
  ...
  for (const [key, value] of Object.entries(rawEffects)) {
    ...
    // Check if this effect exists in processed
    if (processedEffects[key] === undefined) {
      // Check scale significance
  ...
  // Check for extra effects in processed that aren't in raw
...
function main() {
  ...
  const poolIds = poolFilter ? [poolFilter] : Object.keys(poolsData);
  ...
  if (jsonOutput) {
    ...
    const outputPath = path.join(outputDir, 'pool-powers.json');
