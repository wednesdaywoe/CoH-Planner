...
- [x] Investigate the 80+ "Conditional"-labeled Rebirth pet conditional gates — audited 2026-05-03. The literal `"Conditional"` label class is **already resolved** (0 entries in current data; the earlier curation pass collapsed it to 0). Remaining label noise on Rebirth is `"state"` in `specialEffects[]` (chance procs labeled `kind: 'grant'`), 122 entries breaking down as:
    - **21 entries** — fixable by converter relaxation. The Null-grant heuristic was filtering out global-scope conditionals as grant-pair candidates (originally to avoid pairing against AT-inherent meters like Domination). But many self-state grants (Brute Psionic Melee → Insight, Combo-Level stacks, etc.) are global-scope by design. Relaxed the filter to keep all conditionals except the explicit `AT_INHERENT_GRANT_BLACKLIST` set (`domination`). Applied in [convert-powerset.cjs](scripts/convert-powerset.cjs) `extractSpecialEffects` Null branch. **Pending data regen to surface** — the next `convert-all-powersets --dataset=rebirth --force` run carries these into the generated tree.
    - **75 entries** — not fixable at the converter level. Root cause is Parse6 parser limitation: HC's `Grant_Power` template carries the granted dotted name in `params.power_names[0]` (e.g. `Temporary_Powers.Temporary_Powers.Stalker_Assassins_Focus`); Rebirth's Parse6 binary stores the same `Power` field in the AttribMod tail block but [_powers.py:1025](tools/bin-crawler/bin_crawler/parser/_powers.py#L1025) currently `skip_to_end()`'s the tail and surfaces these grants as `Null` attribs with no `params`. Three populations:
        - 49 Stalker primary melee powers → grant `Assassin's Focus`
        - 23 Radiation Melee powers (Brute / Stalker / Tanker / Scrapper) → grant `Contaminated`
        - 3 Mastermind Beast Mastery summons → grant `Pack Mentality`
      Fix path: extend `_parse_effect_template_parse6` to scan the tail for the Power param string-array (analogous to Parse7's `_extract_params` heuristic but adapted to Parse6's inline-pascal string layout). Tracked as a follow-up under Parse6 parser work.
    - **1 entry** — multi-conditional power, falls back to `'state'` because the heuristic can't disambiguate. Negligible.
...
- [x] **Guardian placeholder in Rebirth's archetype dropdown** (2026-05-03). Rebirth has a Guardian AT (Guardian_Assault primary, Guardian_Comp secondary) but its class definition isn't in either `classes.bin` or `villain_classes.bin` — HP tables, damage modifiers, inherent are unsourced. Adding the AT properly is deferred. To indicate the omission is intentional, [Header.tsx](src/components/layout/Header.tsx) now uses `ARCHETYPE_OPTIONS_REBIRTH` (no Sentinel; disabled Guardian "Coming Soon" entry in its slot) when `build.serverId === 'rebirth'`.
- [ ] **Source Guardian's class data** to ship the AT for real. Either find where Rebirth defines `Class_Guardian` (other piggs / server source / `*.def` files) or stub stat tables from a similar-shape AT (Sentinel/Defender) and accept some calc inaccuracy. ~30 min for the stub path; investigation TBD for the proper path.
